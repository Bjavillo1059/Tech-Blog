...
router.get("/dashboard", withAuth, async (req, res) => {
  try {
    let result = await User.findByPk(req.session.userId, {
      include: { model: Blog },
    });
...
router.get("/editpost/:id", withAuth, async (req, res) => {
  try {
    let result = await Blog.findByPk(req.params.id, {
      include: { model: User, attributes: ["userName"] },
    });
...
router.get("/blog/:bid/comment/", withAuth, async (req, res) => {
  try {
    let result = await Blog.findByPk(req.params.bid, {
      include: [
        { model: User, attributes: ["userName"] },
        { model: Comment, include: { model: User, attributes: ["userName"] } },
      ],
...
router.get("/blog/comment/:id", withAuth, async (req, res) => {
  try {
    let commentResult = await Comment.findByPk(req.params.id, {
      include: [
        { model: User, attributes: ["userName"] },
